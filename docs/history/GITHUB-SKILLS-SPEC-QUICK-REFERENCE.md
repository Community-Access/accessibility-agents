...
> Original path: `docs/GITHUB-SKILLS-SPEC-QUICK-REFERENCE.md`.

## Frontmatter Comparison

Each field, with its required, current status, example and priority.
...
Each platform, with its tool reference, file location and status.
...
Each setting, with its current, target and action.
...
Each field, with its current, target and source.
...
Each item, with its action, owner, timeline and blocking.
