...
> Original path: `docs/PHASE-2-DESCRIPTION-REMEDIATION.md`.
...
Each row, with its #, skill, current, target, diff and status.
...
Each row, with its #, skill, current and status.
...
Each skill, with its current and target description.
