...
Each row, with its #, agent, domain, audit domains and writes code.
...
Each row, with its #, agent, domain, standards and writes code.
...
Each row, with its #, agent, domain, key knowledge and writes code.
...
Each row, with its #, agent, role and key functions.
...
Each rule ID, with its severity and description.
...
Each rule ID, with its severity, matterhorn checkpoint and description.
...
Each workflow, with its trigger and purpose.
...
Each file, with its platform and description.
...
Each platform, with its mechanism and schedule.
...
Each standard, with its coverage and notes.
...
Each package, with its version and purpose.
...
Each directory/file, with its contents.
...
Each risk, with its impact and mitigation.
