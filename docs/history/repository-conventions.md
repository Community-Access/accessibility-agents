...
> Original path: `docs/repository-conventions.md`.
>
> Moved from `AGENTS.md` in 7.0. `AGENTS.md` is now the always-on
> instruction contract that every client loads each turn, so it carries only
> the enforcement rule, the router list and the standards. Everything about
> how to write and build files in this repository lives here.
>
...
Each platform, with its read, edit, search, shell and sub-agent.
...
Each workflow, with its purpose.
