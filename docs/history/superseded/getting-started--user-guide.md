...
Each layer, with what it is and count.
...
Each feature, with its VS code and CLI.
...
Each platform, with its location and format.
...
Each prompt, with what it does and when to use it.
...
Each tool, with input, what it does and example usage.
...
Each agent category, with file types they work with and how they access files.
...
Each feature, with its copilot (vs code), copilot CLI, claude code, gemini CLI, codex CLI and claude desktop.
