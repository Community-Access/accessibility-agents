...
See [CI Integrity Guards](../history/ci-integrity-guards.md) for local commands and extension patterns.
...
Then register the hooks in `~/.claude/settings.json` (see the [Hooks Guide](../reference/hooks.md) for the full JSON).
...
If agents are missing or hooks are not firing, see the [Agent Debug Panel Guide](../history/agent-debug-panel.md) for troubleshooting workflows.
...
Each feature, with its claude code and GitHub copilot.
...
Each feature, with its VS code extension and copilot CLI.
...
For the current subagent list and installation details, see [Codex Subagents](../history/codex-experimental-multi-agent.md). For extension authoring, see [Accessibility Agents Extensions](../guides/accessibility-agent-extensions.md).
